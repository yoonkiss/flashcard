...
application.config.update(dict(
    DATABASE=os.path.join(application.root_path, 'db', 'cards.db'),
    SECRET_KEY='development key',
    USERNAME='admin',
    PASSWORD='default'
))
application.config.from_envvar('CARDS_SETTINGS', silent=True)
...
def init_db():
    db = get_db()
    with application.open_resource('data/schema.sql', mode='r') as f:
...
@application.route('/filter_cards/<filter_name>')
...
@application.route('/edit/<card_id>')
def edit(card_id):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    db = get_db()
...
@application.route('/edit_card', methods=['POST'])
def edit_card():
    if not session.get('logged_in'):
        return redirect(url_for('login'))
...
@application.route('/delete/<card_id>')
def delete(card_id):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    db = get_db()
...
@application.route('/general/<card_id>')
...
@application.route('/code/<card_id>')
...
@application.route('/eng/<card_id>')
def eng(card_id=None):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    return memorize("eng", card_id)


@application.route('/ielts')
@application.route('/ielts/<card_id>')
def ielts(card_id=None):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    return memorize("ielts", card_id)
...
@application.route('/mark_known/<card_id>/<card_type>')
